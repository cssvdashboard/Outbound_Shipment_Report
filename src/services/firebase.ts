...
const DEFAULT_FIREBASE_CONFIG: FirebaseConfig = {
  apiKey: "AIzaSyCvR0lYrn8BE2TyTc3SwxFz0Pgtnql9pyw",
  authDomain: "outbound-shipment-report.firebaseapp.com",
  projectId: "outbound-shipment-report",
  storageBucket: "outbound-shipment-report.firebasestorage.app",
  messagingSenderId: "652729889043",
  appId: "1:652729889043:web:0c143bb0cdc05358466ae6",
  measurementId: "G-F1KX2JZQQH"
};

/**
 * Retrieves saved Firebase configuration from browser IndexedDB or localStorage,
 * with fallback to Vite environment variables or default project credentials.
 */
export async function getSavedFirebaseConfig(): Promise<FirebaseConfig | null> {
  try {
    const saved = await get<FirebaseConfig | null>(STORAGE_KEY_FIREBASE_CONFIG);
    if (saved === null) {
      // User explicitly clicked Disconnect
      return null;
    }
  ...
  // Auto-connect with default shared project config
...
export async function clearFirebaseConfig(): Promise<void> {
  ...
  try {
    localStorage.setItem(STORAGE_KEY_FIREBASE_CONFIG, 'disconnected');
  } catch {}
  updateStatus({
    state: 'disconnected',
    projectId: undefined,
...
export async function saveShipmentEditToCloud(
  ...
  try {
    ...
    const payload = {
      ...
      lastEditor: editor || 'Collaborator'
    };
